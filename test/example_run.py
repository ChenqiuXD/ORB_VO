    ...
    for pic_name in os.listdir('../data/rgb'):
        ...
        if orb_detector.match.__len__() != 0:
            ...
            for match in orb_detector.best_matches:
                ...
                # If the depth information is calculated out of range, delete it from matches
                if depth >= orb_detector.max_dis or depth <= orb_detector.min_dis:
                    orb_detector.best_matches.remove(match)
                    continue
                # If the color of the pixel is too white,
                # indicating that it might have wrong depth information, delete it
                if sum(orb_detector.first_color_frame[img_pixel[1], img_pixel[0]]) >= WHITE_THRE:
                    orb_detector.best_matches.remove(match)
                    continue
                ...
                orb_detector.camera_pixel_first.append(point_a_pixel)
            ...
            print(str(iterCount) + ' ' + pic_name + '\n')

            if len(orb_detector.camera_coordinate_first) != 0:
                ...
                if PLOT_TRAJECTORY:
                    ...
                    for j in range(len(orb_detector.camera_coordinate_first)):
                        text = str([change_format(k) for k in orb_detector.camera_coordinate_first[j]]) + \
                               str([change_format(k_) for k_ in orb_detector.camera_pixel_first[j]])
                        # cv2.putText(image, text, tuple([int(i) for i in orb_detector.camera_pixel_first[j][:2]]),
                        #             cv2.FONT_HERSHEY_PLAIN, 1.0, (0, 0, 255), 2)
                        print(text)
                else:
                    text = str(orb_detector.displace_mat[0, 3]) +\
                           ' ' + str(orb_detector.displace_mat[2, 3]) +\
                           ' ' + str(math.atan2(orb_detector.displace_mat[1, 0], orb_detector.displace_mat[0, 0]))
