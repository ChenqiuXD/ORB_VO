...
if __name__ == "__main__":
    ...
    while iterCount < MAX_ITER:
        ...
        if iterCount == 0:
            orb_detector = ORBDetector(depth_intrin=depth_intrin,use_lm=USE_LM,min_dis=MIN_DIS,max_dis=MAX_DIS)
            ...
            if orb_detector.match.__len__() != 0:
                orb_detector.simple_match_filter(threshhold=0.05*GAP)
            else:
                print("初始关键点不足")
                continue

            # Optimize to calculate the transition matrix
            if len(orb_detector.camera_coordinate_first) >= 4:
                ...
                if USE_LM:
                    if not PLOT_TRAJECTORY:
                        ...
                        result = str(ORBDetector.tm[0, 3]) + ' ' + str(ORBDetector.tm[1, 3])
