...
if __name__ == "__main__":
    ...
    while iterCount < MAX_ITER:
        ...
        if iterCount == 0:
            orb_detector = ORBDetector(depth_intrin=depth_intrin, use_lm=USE_LM, max_dis=MAX_DIS, min_dis=MIN_DIS)
            ...
            if len(orb_detector.camera_coordinate_first) >= 3:
                ...
                if USE_LM:
                    ...
                    if not PLOT_TRAJECTORY:
                        ...
                        result = str(ORBDetector.pp[0]) + ' ' + str(ORBDetector.pp[1])
                print(str(iterCount) + ' ' + result + ' ' + str(len(orb_detector.camera_coordinate_first)))
                f.write(result)
                f.write("\n")
        ...
        if iterCount != 0:
            if len(orb_detector.camera_coordinate_first) >= 3:
                ...
                if PLOT_TRAJECTORY:
                    for i in range(4):
                        text = str([change_format(k) for k in orb_detector.tm[i, :]])
                    ...
                    for j in range(len(orb_detector.camera_coordinate_first)):
                        text = str([change_format(k) for k in orb_detector.camera_coordinate_first[j]]) +\
                               str([change_format(i) for i in orb_detector.camera_pixel_first[j]])
                    ...
                    for i in range(4):
                        text = str([change_format(i) for i in orb_detector.displace_mat[i, :]])
                        cv2.putText(image, text, (40, 50 + 20 * i), cv2.FONT_HERSHEY_PLAIN, 1.2, (0, 0, 255), 2)
